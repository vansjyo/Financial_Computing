...
class Sudoku {
    ...
    bool solve() {
        ...
        for(int k = 1; k <= sudokuSize; k++) {
            ...
            // remove the element from there and try placing the next number since it didnt work out
            sudoku[row][col] = 0;
        }

        // if no value of k satisfied i,j, then it cannot be solved for the currently placed values, so backtrack
        return false;
    }
...
public:
    ...
    void solveSudoku() {
        ...
        if (solve()==true)
			...
			cout << "No solution to the Dutch Sudoku Problem" << endl;
    }
};